...
public class Lab1 {
  ...
  public static final EV3Ult            rasonicSensor usSensor = new EV3UltrasonicSensor(SensorPort.S1);
  /** The left motor. */
  ...
  public static void controller(int distance, int[] motorSpeeds) {
    ...
    int error = distance - WALL_DIST; 
    int controllerGain = 5;
    ...
    if (distance < MAX_SENSOR_DIST) {
      ...
      if (Math.abs(error) >= WALL_DIST_ERR_THRESH) {
        leftSpeed = MOTOR_HIGH * controllerGain;
      }
      // +ve error means that the robot is too far from the necessary distance
      // Decrease leftSpeed 
      else if (error > 0 && error != 0) {
        leftSpeed = MOTOR_HIGH - (controllerGain * error);
      } 
      // -ve error means that the robot is too close to the necessary distance 
      // Increase leftSpeeds
      else if (error < 0 && error != 0) {
        leftSpeed = MOTOR_HIGH + (controllerGain * Math.abs(error));
      }
    }
